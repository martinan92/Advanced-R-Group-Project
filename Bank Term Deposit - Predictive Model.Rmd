...
if(file.exists("./Data/smote_train.csv")) {
  ...
  smote_train_df <- fix_types(fread("./Data/smote_train.csv"))
} else {
...
if(file.exists("./Data/smote_train_no_duration.csv")) {
  print("SMOTE already created, reading from file...")
  smote_train_df <- fix_types(fread("./Data/smote_train_no_duration.csv"))
} else {
  smote_train_df <- SMOTE(y ~ ., train_to_smote, perc.over = 500)
...
# Predict on Test Set
```{r}
scaled_validation<-scale_df(raw_bank_test)
clustered_validation <- clustering(df_train=smote_train_df, df_test=scaled_validation, n_clusters=4)

prediction<-predict(logit_cluster, data = clustered_validation, type='raw')
