...
prep_train_data<-fread('Data/BankCamp_train.csv', stringsAsFactors = F)
prep_test_data<-fread('Data/BankCamp_test.csv', stringsAsFactors = F)
...
prep_train_data <- fix_types(prep_train_data)
prep_test_data <- fix_types(prep_test_data)
prep_test_data$y <- NULL
...
prep_train_data <- scale_df(prep_train_data)
prep_test_data <- scale_df(prep_test_data)

```

### 8.3 Saving the prepared datasets 

```{r}
fwrite(prep_train_data, 'Data/prep_train_data.csv', row.names = F)
fwrite(prep_test_data, 'Data/prep_test_data.csv', row.names = F)
```
