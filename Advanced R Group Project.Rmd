...
if(file.exists("./saved_models/no_duration/logit_numerical_combination.rds")) {
  print("Model already trained, reading from file...")
  logit_drop_pdays <- readRDS("./saved_models/no_duration/logit_numerical_combination.rds")
} else { 
  print("Saved model not found, starting training process...")
  pp <- c(scale_df)
  fc <- c(num_combinations)
  logit_numerical_comb <- pipeline_casero(smote_train_df, preprocessing=pp, creation=fc, model="glm")
  saveRDS(logit_numerical_comb, "./saved_models/no_duration/logit_numerical_combination.rds")
}

model_results <- rbind(model_results,
                       evaluate_model(model=logit_numerical_comb,
                                      holdout=scale_df(num_combinations(holdout)),
                                      "logit_numerical_comb"))
...
if(file.exists("./saved_models/no_duration/logit_combined.rds")) {
  print("Model already trained, reading from file...")
  logit_drop_day_pdays <- readRDS("./saved_models/no_duration/logit_combined.rds")
} else { 
  print("Saved model not found, starting training process...")
  pp <- c(scale_df)
  fs <- c(drop_day)
...
model_results <- rbind(model_results,
                       evaluate_model(model=logit_combination,
                                      holdout=num_combinations(holdout),
                                      "logit_combination"))
...
if(file.exists("./saved_models/no_duration/rf_numerical_comb.rds")) {
  print("Model already trained, reading from file...")
  rf_drop_day_pdays <- readRDS("./saved_models/no_duration/rf_numerical_comb.rds")
} else {
  print("Saved model not found, starting training process...")
  tg <- data.table(expand.grid(mtry=15,
                             splitrule='gini',
...
model_results <- rbind(model_results, evaluate_model(model=rf_numerical_comb, holdout=holdout, "rf_numerical_comb"))
...
model_results <- rbind(model_results, evaluate_model(model=rf_combined, holdout=holdout, "rf_combined"))
...
### FE5: Clustering
The fourth engineered feature combines the previous clustering feature and dropping the day column.
```{r}
if(file.exists("./saved_models/no_duration/logit_cluster.rds")) {
  ...
  logit_cluster_drop_day <- pipeline_casero(clustered$train, preprocessing=pp, selection=fs, creation=fc, model="glm")
