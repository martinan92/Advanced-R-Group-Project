...
## Fixing Data Types and Scaling
```{r}
fixed_train_df <- fix_types(raw_bank_train)

scaled_train_df <- scale_df(fixed_train_df)
scaled_test_df <- scale_df(raw_bank_test)

head(scaled_train_df)
```

## SMOTE
```{r}
scaled_test_df$y <- ifelse(scaled_test_df$y=='no',0,1)
scaled_test_df$y <- as.factor(scaled_test_df$y)

smote_train_df <- SMOTE(y ~ ., scaled_train_df, perc.over = 500)
